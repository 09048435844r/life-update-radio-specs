...
Phase 5（v1.6 / 2026-05-07 〜 2026-05-09 実装予定）: 信頼度メタデータの追加
[ ] structured_facts に cross_validated_sources を追加
[ ] structured_facts に confidence を追加
[ ] structured_facts に flags を追加（highly_specific のみ）
[ ] _merge_structured_facts の dedup 拡張
[ ] confidence 判定ロジック実装
期待効果: 台本側のハルシネーション抑制、radio_director の Phase B/D 設計に直結

Phase 6（v1.7+ 議論予定）: 拡張フラグと publication_date
...
10.3 v1.6 が radio_director 設計に与える影響
confidence フィールド:
  → Phase B プロンプトでの優先度ロジックに反映
    （high の claim を優先的に台本に組み込む）
  → Phase D での検証時、low confidence の claim 使用を警告

cross_validated_sources フィールド:
  → 「複数ソースで確認できた事実」として台本で表現可能
    （「複数の研究が示唆するように」等）

flags["highly_specific"]:
  → Phase D の設計に直接活用
  → 同じパターンを台本中の発言にも適用してハルシネーション検出
